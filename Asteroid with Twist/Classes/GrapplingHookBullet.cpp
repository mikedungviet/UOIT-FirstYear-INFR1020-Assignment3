 ...
 */
GrapplingHookBullet::GrapplingHookBullet(const SpaceShip * ar_Ship): GameEntities("EnemyBullet"), pr_LifeTime(3.f)
{
...
void GrapplingHookBullet::Update(const float& ar_DeltaTime)
{
	...
	if (pr_LifeTime < 0) {
		...
		GameEntitiesSingleton::GetInstance()->GetSpaceShip()->ChangeToNormalState();
	}
}
